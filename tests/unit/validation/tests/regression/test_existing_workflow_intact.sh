...
# Change to project root (assuming script is in tests/regression/)
cd "$(dirname "$0")/../.."
...
NEW_FILES=(
    ".claude/skills/thesis-orchestrator/scripts/workflow_validator.py"
    ".claude/skills/thesis-orchestrator/scripts/validated_executor.py"
)

for file in "${NEW_FILES[@]}"; do
    if [ -f "$file" ]; then
        echo -e "${GREEN}✅ OK: $file is a new file${NC}"
    else
        echo -e "${YELLOW}⚠️  WARNING: $file not found (might not be created yet)${NC}"
    fi
done
...
for file in "${NEW_FILES[@]}"; do
    if [ -f "$file" ]; then
        if grep -q "from.*init_session\|from.*checklist_manager\|from.*context_loader" "$file" 2>/dev/null; then
...
if [ $INDEPENDENT -eq 1 ]; then
    echo -e "${GREEN}✅ OK: New files are independent${NC}"
fi
